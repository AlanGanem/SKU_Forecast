...
X_train, y_train, X_val, y_val = TSU.chunk_and_concatenate_dict(train_data_dict,pred_period,look_back_period,encoder_inputs,decoder_inputs,n_validation_intervals = n_validation_intervals, scaler = False)
...
sample_weights = y_train[:,:,0]**(1/3)
train_history = model.fit([X_train,X_covars_train], y_train, batch_size = batch_size, epochs = 5000, validation_data =[[X_val,X_covars_val], y_val] ,callbacks = [es],sample_weight = sample_weights)
predictor = Model(inputs = [encoder_input,decoder_input],outputs = mu)
...
for col in pred_true_df.columns:
    error_df[str(col)] = abs(pred_true_df[col]-pred_true_df['Volume'])/pred_true_df['Volume']

error_df.median()
error_df.std()
# sanity_check
plt.plot(preds_df.index,preds_df.actual)
